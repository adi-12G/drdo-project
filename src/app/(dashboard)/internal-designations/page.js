...
export default function InternalDesignationsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    await apiFetch("/internal-designations", {
      method: "POST",
  ...
  const deleteDesignation = async (id) => {
    await apiFetch(`/internal-designations/${id}`, { method: "DELETE" });
    fetchDesignations();
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <button className="bg-[#0F4C5C] text-white px-4">
      ...
      <table className="w-full bg-white border">
        ...
        <tbody>
          {designations.map((designation) => (
            <tr key={designation.internal_designation_id}>
              <td className="border p-3 text-[#073B4C]">
                {designation.internal_designation_id}
              </td>
              <td className="border p-3 text-[#073B4C]">{designation.sname}</td>
              <td className="border p-3 text-[#073B4C]">
              ...
              <td className="border p-3 text-[#073B4C]">
                <button
                  onClick={() =>
                    deleteDesignation(designation.internal_designation_id)
                  }
      ...
      <p className="text-sm text-gray-600 mt-3">
        Update is not supported by the backend for internal designations, so this page currently supports list, create, and delete.
      </p>
    </div>
  );
}
